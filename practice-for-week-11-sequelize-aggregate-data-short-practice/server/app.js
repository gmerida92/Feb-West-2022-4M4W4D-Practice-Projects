...
app.get("/toys", async (req, res, next) => {
	...
	// const toysCount = await allToys.count(); ONLY WORKS WITH MODEL METHOD
...
app.get("/cats/:id/toys", async (req, res, next) => {
	const catToysAggregateData = await Cat.findByPk(req.params.id, {
		...
		attributes: [
			//When specifying column, make sure to use plural ref TOYS
	...
	const catData = cat.toJSON(); // changes object to json object
